...
//    新增留言
    ...
    public String post(Message message, HttpSession session, Model model) {
        ...
        //设置头像
        /*if (user != null&&user.getType()==1&&user.getNickname().equals(message.getNickname())) {
            ...
            List<Message> messages = messageService.listMessage();
            model.addAttribute("messages", messages);
        } else if(user != null&&user.getType()==2&&user.getNickname().equals(message.getNickname())){
            if (message.getParentMessage().getId() != null) {
                message.setParentMessageId(message.getParentMessage().getId());
            }
            message.setAvatar(user.getAvatar());
        ...
        if (user != null&&user.getNickname().equals(message.getNickname())&&user.getEmail().equals(message.getEmail())){
            message.setAvatar(user.getAvatar());
            if (user.getType()==1){
                message.setAdminMessage(true);
...
//    删除留言
    ...
    public String delete(@PathVariable Long id, RedirectAttributes attributes, Model model){
        ...
        return "redirect:/templates/message";
    }
